...
test.describe('Greenhouse - Dedicated Job Page', () => {
  let page;

  test.beforeEach(async ({ browser }) => {
    page = await browser.newPage();

    const htmlPath = path.join(__dirname, '../fixtures/greenhouse-dedicated-job-page.html');
    ...
    await page.goto('https://job-boards.greenhouse.io/discord/jobs/8571766002');
  ...
  test('selects job description on keyboard shortcut', async () => {
    ...
    expect(selectedText).toContain('About the team');
    expect(selectedText).toContain('trust and safety tooling');
    expect(selectedText).toContain('10+ years of engineering leadership experience');
  });
  ...
  test('findJobTitleUrl returns current URL on dedicated page', async () => {
    ...
    expect(url).toContain('/discord/jobs/8571766002');
  });
});
